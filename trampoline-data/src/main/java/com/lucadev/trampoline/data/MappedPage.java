 ...
 * <p>
 * This class allows you to easily map a {@link Page} type into another type. Useful when
 * wanting to return a page of DTOs instead of JPA entities in your REST endpoint.
 *
...
public class MappedPage<R> extends PageImpl<R> {
	 ...
	 */
	public <T> MappedPage(Page<T> original, Pageable pageable,
			Function<T, R> mapFunction) {
		super(original.getContent().stream().map(mapFunction)
				.collect(Collectors.toList()), pageable, original.getTotalElements());
	}

	/**
	 * Static method which just invokes the constructor. Might make your code look nicer.
	 * @param original the original Page before
	 * @param mapFunction the function to transform the contents of the page.
	 * @param <T> the type of the original page.
